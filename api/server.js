...
const admins = [];
let activeChats = []; // for one to one communication
...
io.on("connection", (socket) => {
    ...
    //* broadcast emit emits the message to all of the clients except the sender,
    ...
    socket.on("disconnect", (reason) => {
        // admin disconnect
        const removeIndex = admins.findIndex((item) => item.id === socket.id); // refreshing page or user closes the browser
        if (removeIndex !== -1) {
